...
const EventRegistry = ({ userRole }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 italic">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 italic pb-20">
        {events.map(ev => (
          <div key={ev.id} className="bg-slate-900/80 border border-slate-800 p-8 rounded-[2rem] space-y-6 shadow-2xl relative overflow-hidden group hover:border-green-500/30 transition-all duration-500 italic">
            <div className="flex justify-between items-start italic relative z-10">
              <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest border italic transition-all ${ev.is_active ? 'bg-green-500/10 text-green-500 border-green-500/20 shadow-[0_0_15px_rgba(34,197,94,0.1)]' : 'bg-slate-950 text-slate-700 border-slate-800 opacity-50'}`}>{ev.is_active ? 'ACTIVE' : 'OFFLINE'}</span>
              {isAdmin && <button onClick={() => navigate(`/event/${ev.id}/analytics`)} className="p-2 bg-slate-950 rounded-lg text-slate-600 hover:text-blue-400 transition-all active:scale-90 italic shadow-xl"><BarChart3 size={18}/></button>}
            </div>
            <div className="space-y-1 italic relative z-10">
              <h3 className="text-2xl font-black italic text-white uppercase tracking-tighter leading-none group-hover:text-green-400 transition-colors duration-500 italic">{ev.title}</h3>
