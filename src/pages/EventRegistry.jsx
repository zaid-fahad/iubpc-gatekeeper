    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-400 text-xs font-bold uppercase tracking-[0.2em] mt-2">Manage and monitor all active events</p>
        </div>
        {isAdmin && (
          <button 
            onClick={() => setShowEventModal(true)} 
            className="px-6 py-3 bg-green-500 text-slate-950 rounded-xl font-black text-xs flex items-center gap-2 shadow-2xl shadow-green-500/20 active:scale-95 transition-all"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
        {events.map(ev => (
          <div key={ev.id} className="bg-slate-900/80 border border-slate-800 p-8 rounded-[2rem] space-y-6 shadow-2xl relative overflow-hidden group hover:border-green-500/30 transition-all duration-500">
            <div className="flex justify-between items-start relative z-10">
              <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${ev.is_active ? 'bg-green-500/10 text-green-500 border-green-500/20 shadow-[0_0_15px_rgba(34,197,94,0.1)]' : 'bg-slate-950 text-slate-500 border-slate-800 opacity-50'}`}>{ev.is_active ? 'ACTIVE' : 'OFFLINE'}</span>
              {isAdmin && <button onClick={() => navigate(`/event/${ev.id}/analytics`)} className="p-2 bg-slate-950 rounded-lg text-slate-400 hover:text-blue-400 transition-all active:scale-90 shadow-xl"><BarChart3 size={18}/></button>}
            </div>
        ...
        {events.length === 0 && !loading && (
          <div className="md:col-span-2 p-24 border-2 border-dashed border-slate-800 rounded-[4rem] text-center">
            <Calendar size={64} className="mx-auto text-slate-800 mb-6" />
            <p className="text-slate-400 font-black uppercase tracking-[0.3em] text-base">No events found. Create a new event to begin.</p>
          </div>
        )}
      </div>
      ...
      {showEventModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl animate-in fade-in duration-300">
          <form onSubmit={handleCreateEvent} className="relative bg-slate-900 border border-slate-800 rounded-[3rem] p-12 w-full max-w-lg space-y-8 shadow-2xl text-left">
            <div className="flex justify-between items-center"><h2 className="text-3xl font-black text-white tracking-tighter uppercase leading-none italic">New Event</h2><X className="text-slate-400 cursor-pointer hover:text-white transition-colors" onClick={() => setShowEventModal(false)} /></div>
            <div className="space-y-5 text-left">
              <div className="space-y-2"><label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Event Name</label><input value={newEvent.title} onChange={e => setNewEvent({...newEvent, title: e.target.value})} placeholder="e.g. Annual Gala 2026" required className="w-full bg-slate-950 border border-slate-800 p-5 rounded-2xl text-base font-bold text-white outline-none focus:ring-1 focus:ring-green-500 shadow-inner uppercase tracking-widest" /></div>
              <div className="space-y-2"><label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Event Date</label><input type="date" value={newEvent.date} onChange={e => setNewEvent({...newEvent, date: e.target.value})} required className="w-full bg-slate-950 border border-slate-800 p-5 rounded-2xl text-base text-white font-bold outline-none shadow-inner" /></div>
            </div>
