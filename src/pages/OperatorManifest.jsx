  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h2 className="text-3xl font-black text-white uppercase tracking-tighter italic">Staff List</h2>
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mt-2">Manage staff permissions and access</p>
        </div>
        <button 
          onClick={() => setShowUserModal(true)} 
          className="px-6 py-3 bg-blue-500 text-slate-950 rounded-xl font-black text-[10px] flex items-center gap-2 shadow-2xl shadow-blue-500/20 active:scale-95 transition-all w-fit"
        >
      ...
      <div className="grid grid-cols-1 gap-3 pb-20">
        {filteredUsers.map(u => (
          <div key={u.id} className="bg-slate-900/60 border border-slate-800 p-5 rounded-2xl flex items-center justify-between group hover:border-blue-500/30 transition-all shadow-xl relative overflow-hidden">
      ...
      {showUserModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl animate-in fade-in duration-300">
          <form onSubmit={handleCreateUser} className="relative bg-slate-900 border border-slate-800 rounded-[2.5rem] p-10 w-full max-w-md space-y-6 shadow-2xl text-left">
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                    <UserPlus className="text-blue-500" size={24} />
                    <h2 className="text-2xl font-black text-white tracking-tighter uppercase leading-none italic">Add Staff</h2>
                </div>
                <button type="button" onClick={() => setShowUserModal(false)} className="p-2 text-slate-600 hover:text-white transition-colors">
                    <X size={20} />
                </button>
            </div>
            <div className="space-y-4 text-left">
              {(unassignedUsers.length > 0 || fetchingUnassigned) ? (
                <div className="space-y-1.5 p-4 bg-blue-500/5 border border-blue-500/20 rounded-2xl animate-in zoom-in-95 duration-300">
                  <div className="flex items-center gap-2 mb-2">
                    <UserSearch size={14} className="text-blue-500" />
                    <label className="text-[9px] font-black uppercase tracking-widest text-blue-500">Select from Registered Users</label>
                  </div>
                  <select 
                    onChange={e => {
                      if (e.target.value) {
                    ...
                    className="w-full bg-slate-950 border border-slate-800 p-3 rounded-xl text-[10px] font-bold text-white outline-none focus:ring-1 focus:ring-blue-500 shadow-inner uppercase tracking-widest disabled:opacity-50"
                  >
                ...
                <div className="p-6 bg-slate-950/50 border border-slate-800/50 rounded-2xl text-center space-y-3">
              ...
              <div className="space-y-1.5 relative">
                ...
                <div className="relative">
                  ...
                  <input value={newUser.full_name} onChange={e => setNewUser({...newUser, full_name: e.target.value})} placeholder="FULL NAME" required className="w-full bg-slate-950 border border-slate-800 p-4 pl-12 rounded-xl text-xs font-bold text-white outline-none focus:ring-1 focus:ring-blue-500 shadow-inner uppercase tracking-widest" />
                </div>
              </div>
              <div className="space-y-1.5 relative">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-600 ml-1">Email Address</label>
                <div className="relative">
                  <ShieldCheck className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-700" size={16} />
                  <input type="email" value={newUser.email} onChange={e => setNewUser({...newUser, email: e.target.value})} placeholder="name@iub.edu.bd" required className="w-full bg-slate-950 border border-slate-800 p-4 pl-12 rounded-xl text-xs font-bold text-white outline-none focus:ring-1 focus:ring-blue-500 shadow-inner uppercase tracking-widest" />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-[9px] font-black uppercase tracking-widest text-slate-600 ml-1">Role</label>
                <select value={newUser.role} onChange={e => setNewUser({...newUser, role: e.target.value})} className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl text-xs font-bold text-white outline-none focus:ring-1 focus:ring-blue-500 shadow-inner uppercase tracking-widest appearance-none">
                  <option value="volunteer">Volunteer (Check-in Only)</option>
                  <option value="admin">Administrator (Full Access)</option>
                </select>
              </div>
            </div>
            <button className="w-full py-4 bg-blue-500 text-slate-950 font-black rounded-xl uppercase tracking-widest shadow-2xl shadow-blue-500/20 active:scale-95 transition-all border-b-4 border-blue-700 text-[10px]">ADD STAFF MEMBER</button>
          </form>
        </div>
      )}
    </div>
